...
static av_cold int ffaml_init_decoder(AVCodecContext *avctx)
{
  ...
  // aml sysfs setup
  amlsysfs_write_int(avctx, "/sys/class/video/disable_video", 0);   // enable video output
  amlsysfs_write_int(avctx, "/sys/class/video/blackout_policy", 0); // disable blackout policy
  amlsysfs_write_int(avctx, "/sys/class/video/freerun_mode", 3);    // enable freerun frame mode
...
static void ffaml_release_frame(void *opaque, uint8_t *data)
{
  ...
  AMLBuffer *buffer = (AMLBuffer*)data;
  AMLDecodeContext *aml_context = (AMLDecodeContext*)opaque;

  if (!buffer)
    return;

  if ((aml_context) && (buffer->free))
  {
    ioctl(aml_context->amv_fd, AMSTREAM_IOC_FREE_FREERUN_FRAME, &buffer->handle);
  }
                         ...
                         AVPacket *avpkt)
{
  ...
  // try to grab a frame handle from decoder
  for (int i=0; i < 10; i++)
  {
    ret = ioctl(aml_context->amv_fd, AMSTREAM_IOC_GET_FREERUN_FRAME, &frame_handle);
    if (ret)
    {
      usleep(5000);
    }
  ...
  if (*got_frame)
  {
    ...
    if (framepacket)
    {
      ...
      buffer = malloc(sizeof(AMLBuffer));
      memset(buffer, 0, sizeof(AMLBuffer));

      buffer->free = 1;
      buffer->handle = frame_handle;

      frame->buf[0] = av_buffer_create((uint8_t *)buffer, sizeof(AMLBuffer), ffaml_release_frame , aml_context, 0);
...
static void ffaml_flush(AVCodecContext *avctx)
{
  ...
  // clear our framequeue
  ffaml_queue_clear(avctx, &aml_context->framequeue);

  // dequeue the frames that could be there, considering that
  // if we have no frames for 100 ms we're good
  count = 0;
  do
  {
    unsigned long frame_handle;

    if (ioctl(aml_context->amv_fd, AMSTREAM_IOC_GET_FREERUN_FRAME, &frame_handle) == 0)
    {
      av_log(avctx, AV_LOG_DEBUG, "Freeing frame %ld ...\n", frame_handle);
      ioctl(aml_context->amv_fd, AMSTREAM_IOC_FREE_FREERUN_FRAME, &frame_handle);
      count = 0;
    }
    else
      count++;
